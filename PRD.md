...
- **Community stats**: Server-owned aggregate published to `community-stats/global`; owner-authored source writes queue exact-schema durable outbox events, a trusted Vercel Function rebuilds from one consistent snapshot across `users`, `playthroughs`, and `campaignRuns`, and the public document is read-only to clients
...
- **Functionality**: When a signed-in owner creates, edits, deletes, imports, promotes, or restores source data, that write atomically emits a durable outbox event. After commit, the client wakes a Firebase-ID-token-authenticated Vercel Function. The trusted worker coalesces pending outbox work, rebuilds community statistics from one snapshot, and publishes the result to `community-stats/global`.
...
- **Retry/follow-up behavior**: The durable outbox remains the authoritative queued state. Signed-in clients retry retryable responses, later signed-in sessions opportunistically wake pending work, and one Hobby-compatible daily Vercel Cron invocation recovers abandoned work. A short Firestore lease prevents concurrent rebuilds. Each invocation performs at most two bounded cleanup passes; larger backlogs remain `stale` and are drained by later wakes. Persistent configuration or operational failures remain explicit as `stale` / `failed` until fixed.
...
- **Rollout order**: Deploy Firestore rules, then deploy the Vercel backend with `VITE_COMMUNITY_STATS_API_ENABLED=false`. Run the explicit bootstrap script under Node 22 against the intended Firebase project and deployed endpoint, wait for schema version 3 plus an empty outbox and the exact bootstrap marker, prove source fingerprints are unchanged, then enable client wakes and redeploy the same revision.
