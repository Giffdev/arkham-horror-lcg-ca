 ...
 *  3. Strip Inkscape/Sodipodi metadata elements (<sodipodi:*>, <inkscape:*>)
 *     and empty <defs> blocks — these non-rendering elements are generated by
 *     Inkscape's SVG export and can disrupt fill-color inheritance when the SVG
 *     is injected via dangerouslySetInnerHTML in a browser context.
 *  4. Remove all fill="..." attributes from every element.
 *  5. Remove class="..." attributes from non-<svg> elements so orphaned
 *     class references can't accidentally pick up external stylesheets.
 *  6. Inject fill="currentColor" on the root <svg> element AND on every SVG
 *     shape element (path, circle, rect, polygon, polyline, ellipse, line).
 *     Belt-and-suspenders: the root attribute establishes the SVG default;
 *     per-element attributes ensure currentColor is used even if browser
 *     fill-inheritance from root doesn't cascade through Inkscape metadata.
 */
function normalise(raw: string): string {
  return raw
    .replace(/<\?xml[^?]*\?>\s*/g, '')
    .replace(/<style\b[^>]*>[\s\S]*?<\/style>/gi, '')
    // Strip Inkscape/Sodipodi metadata elements (self-closing and with content)
    .replace(/<sodipodi:[^>]*\/>/gi, '')
    .replace(/<sodipodi:[^>]*>[\s\S]*?<\/sodipodi:[a-zA-Z]+>/gi, '')
    .replace(/<inkscape:[^>]*\/>/gi, '')
    .replace(/<inkscape:[^>]*>[\s\S]*?<\/inkscape:[a-zA-Z]+>/gi, '')
    // Strip empty <defs> blocks left by Inkscape export
    .replace(/<defs\b[^>]*\/>/gi, '')
    .replace(/<defs\b[^>]*>\s*<\/defs>/gi, '')
    .replace(/\sfill="[^"]*"/g, '')
    .replace(/(<(?!svg\b)[a-zA-Z][^>]*?)\sclass="[^"]*"/g, '$1')
    // Belt-and-suspenders: fill on each shape element (not just SVG root)
